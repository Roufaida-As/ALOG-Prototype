...
dashboard.py
------------
Dashboard web temps réel du SGFF.

Stack : FastAPI + WebSocket + HTML intégré
Ouvrir dans le browser : http://localhost:8000

Rôle : consommer les alertes depuis Kafka et les pousser
       instantanément vers le browser via WebSocket.

Tactiques démontrées :
- Maintain Audit Trail (Bass ch.9) : chaque alerte affichée
  avec horodatage, zone, capteur, niveau.
- Prioritize Events (Bass ch.8) : les alertes critiques
  apparaissent en rouge immédiatement, sans attente.
"""
...
# Liste des clients WebSocket connectés (un par onglet browser ouvert)
clients_connectes: list[WebSocket] = []

# File d'attente thread-safe entre Kafka (thread) et WebSocket (async)
    ...
    <script>
        ...
        ws.onopen = () => {
            document.getElementById("statut").textContent =
                "✅ Connecté — alertes en temps réel";
        };
        ...
        ws.onclose = () => {
            document.getElementById("statut").textContent =
                "❌ Connexion perdue — relancer dashboard.py";
        };
        ...
        function mettreAJourZone(data) {
            ...
            const css  = data.type === "CRITIQUE" ? "critique" :
                         data.type === "NORMAL"   ? "normal" : "ok";
            const card = document.getElementById(`zone-${zone}`);
            if (!card) return;
            card.className = `zone-card ${css}`;
            document.getElementById(`temp-${zone}`).className = `zone-temp ${css}`;
            document.getElementById(`temp-${zone}`).textContent = `${data.temperature}°C`;
            document.getElementById(`meta-${zone}`).textContent =
                `Fumée: ${data.fumee} | ${data.timestamp}`;
        }
    </script>
</body>
</html>
...
async def websocket_endpoint(websocket: WebSocket):
    """
    Le browser se connecte ici.
    Chaque alerte de Kafka est poussée immédiatement au client.
    """
    ...
    try:
        while True:
            ...
            await websocket.send_text(json.dumps(alerte))
    except WebSocketDisconnect:
        clients_connectes.remove(websocket)


# ── KAFKA CONSUMER (thread séparé) ──────────────────────────────────────────

def consommer_kafka(loop):
    """
    Tourne dans un thread séparé pour ne pas bloquer FastAPI.
...
async def startup():
    ...
    threading.Thread(target=consommer_kafka, args=(loop,), daemon=True).start()
